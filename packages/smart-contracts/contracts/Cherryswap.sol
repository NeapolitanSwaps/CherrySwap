...
contract Cherryswap is Initializable {
  ...
  ***************/
    ...
    enum Status {Open, Running, Closed}
    ...
    function initialize(address _token, address _cherryToken, address _swapMath)
        public
        initializer
    {
        require(
            _token != address(0),
            "Cherryswap::contructor - token cannot be 0"
        );
        require(
            _cherryToken != address(0),
            "Cherryswap::constructor - cherryToken cannot be 0"
        );
    ...
    function createSwap(uint256 _startingTime, uint256 _endingTime) public {
        require(
            (_startingTime >= now) && (_endingTime > _startingTime),
            "Cherryswap::error - invalid swap period"
        );

        if (swapsCounter > 0) {
            Swap memory previousSwap = swaps[swapsCounter - 1];
            require(
                now > previousSwap.endingTime,
                "Cherryswap::hasEnded - swap not ended yet"
            );
        }
    ...
    function startSwap() public {
        ...
        Swap memory previousSwap = swaps[swapsCounter - 1];
        require(
            now <= previousSwap.startingTime,
            "Cherryswap::startSwap - swap is running or closed"
        );
        ...
        // get starting rate
        swaps[swapsCounter - 1].startingRate =
            (cherryToken.supplyRatePerBlock() * blockPerYear) /
            10**18;
    }
    ...
    function closeSwap() public {
        ...
        require(
            now >= previousSwap.endingTime,
            "Cherryswap::closeSwap - swap not ended yet"
        );

        swaps[swapsCounter - 1].status = Status.Closed;

        // get starting rate
        swaps[swapsCounter - 1].endingRate =
            (cherryToken.supplyRatePerBlock() * blockPerYear) /
            10**18;
        ...
        // Redeem cDai to Dai
        require(
            cherryToken.redeem(cBalance) == 0,
            "Cherryswap::redeem - something went wrong"
        );
    ...
    function deposit(
        ...
        require(
            now < currentSwap.startingTime,
            "Cherryswap::deposit - swap is running or closed"
        );
        require(_participant != address(0), "Invalid participant address");

        // collect proposal deposit from proposer and store it in the Moloch until the proposal is processed
        require(
            token.transferFrom(_participant, address(this), _depositedValue),
            "Cherryswap::deposit - deposit token transfer failed"
        );
    ...
    function transfer(int256 rLong, int256 rShort) internal {
        ...
        require(
            now >= previousSwap.endingTime,
            "Cherryswap::transfer - swap not ended yet"
        );
        ...
        for (uint256 i = 0; i < endedSwap.participantsCounter; i++) {
            if (endedSwap.bets[i] == Bet.Long) {
                ...
                payout =
                    (int256(pt) *
                        int256(
                            (endedSwap.depositedValues[i] * 10**18) /
                                previousSwap.depositedValue
                        ) +
                        (rLong * int256(endedSwap.depositedValues[i]))) /
                    10**18;
            } else {
                ...
                payout =
                    (int256(pt) *
                        ...
                        (rShort * int256(endedSwap.depositedValues[i]))) /
                    10**18;
            }
